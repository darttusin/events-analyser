...
df = read_parquet_skip_broken_rows(
    "final_data.parquet", columns=["target", "_seq_len", "domain", "user_id"]
)
...
class EventSequenceDataset(Dataset):
    ...
    def tokenize_sequence(self, seq):
        return [
            ...
            for event in seq
            # if event not in ["OFFER_DECLINED", "OFFER_ACCEPTED"]
        ]
...
class GRUEventClassifier(pl.LightningModule):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        packed_output, (h_n, c_n) = self.gru(x)
...
train_dataset = EventSequenceDataset(
    df_train.drop("target", axis=1), df_train["target"], max_len=70
)
...
model = GRUEventClassifier(
    vocab_size=vocab_size, embed_dim=EMBED_DIM, hidden_size=HIDDEN_SIZE, lr=LR, num_layers=NUM_LAYERS
)
...
checkpoint_callback = ModelCheckpoint(
    ...
    mode="max",
    monitor="val_roc_auc",
    filename="lstm_ds2kk_len70_nooffers_64_128{epoch}-{val_roc_auc:.3f}-{val_loss:.3f}-{train_loss_epoch:.3f}-{val_fscore:.3f}",
    save_top_k=1,
)

trainer = pl.Trainer(
    max_epochs=25, callbacks=[early_stopping_callback, lr_monitor, checkpoint_callback],
    accelerator='gpu',
    devices=[1]
)
